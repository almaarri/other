...
class payroll{
	...
	double hourlyRate, hours, salary, taxRate, taxAmount, grossPay, netPay;
   ...
   double calculateTaxAmount(){ taxRate = .30; //set a flat taxrate 30%
      ...
      double calculateNetPay(){
 		//instert net pay function
	   return netPay;    } //end
   void printData(){ //print out the data
    cout<<setprecision(2)<<setiosflags(ios::fixed | ios::showpoint);
 	cout<<employeeID<<"\t"<<firstName<<"\t"<<lastName<<"\t"<<hours<<"\t"<<payStat<<endl;  }
     //end printData() function
}; //end Payroll class
class employeeSalary : public payroll{
	public: 
	double regPay, otPay, grossPay;
	int otHours;
...
void printHeader(){	
	...
	cout<<"------------------------------------------------------------------------------"<<endl;
	cout<<"ID"<<"\t"<<"First"<<"\t"<<"Last"<<"\t"<<"Hours"<<"\t"<<"Stat"<<endl;
cout<<"------------------------------------------------------------------------------"<<endl; 
