...
void sortNetPayUsingExselSort(int netPayArray[], int numberOfEmployees){
int lower, upper, sortflag = 0, sml = 0, scan = 0, temp, i, j;
lower = 0;
...
while( (lower<upper) && (sortflag == 1) ) {
	...
	if( netPayArray[scan] > netPayArray[scan+1] ){
        temp = netPayArray[scan];
        netPayArray[scan]=netPayArray[scan+1];
        netPayArray[scan+1]=temp;
		sortflag = 1;
	    if( netPayArray[scan] <netPayArray[sml] ){
        sml = scan; }
       }
         scan++; } // while
    temp=netPayArray[lower];
 	netPayArray[lower] = netPayArray[sml];
	netPayArray[sml]=temp;
    upper = upper - 1;
     lower = lower +1;  } // while
	   }//exsel sort
int main(void){
   ...
   while (employeeCounter < totalEmployeeCount){
	  ...
	  //input for employee
	    ...
	    printHeader(); //call function to print the header
		employeeCounter = 0;
	   while (employeeCounter < totalEmployeeCount){
	   	employee[employeeCounter]->printData();
	   	employeeCounter++;}
	   	cout<<"Net pay before sort:"<<endl;
	   	employeeCounter = 0;
	   	while (employeeCounter < totalEmployeeCount){
	   		cout<<employee[employeeCounter]->netPay<<endl;
	   		netPayArray[employeeCounter]=employee[employeeCounter]->netPay;
	   		employeeCounter++;}
	   	sortNetPayUsingExselSort(netPayArray, totalEmployeeCount);
	   	cout<<"Net pay after sort:"<<endl;
	   	employeeCounter = 0;
	   	while (employeeCounter < totalEmployeeCount){
	   		cout<<netPayArray[employeeCounter]<<endl;
	   		employeeCounter++;
		   }
}
